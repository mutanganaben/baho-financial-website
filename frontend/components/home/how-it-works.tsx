...
import { FileText, Search, CreditCard, ArrowRight, LucideIcon } from "lucide-react";
...
export const HowItWorks: React.FC = () => {
  return (
    <section className="py-20 lg:py-28 bg-[#0B1B33] text-white overflow-hidden relative border-t border-b border-slate-800">
            ...
            return (
              <div key={index} className="flex flex-col items-center text-center space-y-5 group relative">
                ...
                {index < STEPS.length - 1 && (
                  <div className="hidden md:block absolute top-10 left-[60%] w-[80%] h-0.5 border-t-2 border-dashed border-slate-700 -z-0" />
                )}

                {/* Step Circle Badge */}
                <div className="w-20 h-20 rounded-full bg-baho-gold text-slate-950 flex items-center justify-center p-5 shadow-lg group-hover:scale-110 transition-transform duration-300 relative z-10">
                  <IconComponent className="w-9 h-9" />
                </div>

                {/* Step Number Tag */}
                <div className="text-xs font-extrabold text-baho-gold tracking-widest uppercase">
                  {step.number}
                </div>
                ...
                {/* Step Description */}
                <p className="text-sm text-slate-300 leading-relaxed max-w-xs font-normal">
                  {step.description}
                </p>
              </div>
            );
